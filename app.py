...
nombre_proyecto = st.text_input('Nombre del Proyecto', value=st.session_state.nombre_proyecto)
st.session_state.nombre_proyecto = nombre_proyecto

if "nombre_titular_proyecto" not in st.session_state:
    st.session_state.nombre_titular_proyecto = ""
nombre_titular_proyecto = st.text_input('Nombre del Titular del proyecto', value=st.session_state.nombre_titular_proyecto)
st.session_state.nombre_titular_proyecto = nombre_titular_proyecto

if "contexto_proyecto" not in st.session_state:
    st.session_state.contexto_proyecto = ""
contexto_proyecto = st.text_area('Contexto del Proyecto', value=st.session_state.contexto_proyecto)
st.session_state.contexto_proyecto = contexto_proyecto

if "objetivo_proyecto" not in st.session_state:
    st.session_state.objetivo_proyecto = ""
objetivo_proyecto = st.text_area('Objetivo del Proyecto', value=st.session_state.objetivo_proyecto)
st.session_state.objetivo_proyecto = objetivo_proyecto

if "metricas_exito" not in st.session_state:
    st.session_state.metricas_exito = ""
metricas_exito = st.text_area('Metricas de Exito', value=st.session_state.metricas_exito)
st.session_state.metricas_exito = metricas_exito

if "experiencia_titular" not in st.session_state:
    st.session_state.experiencia_titular = ""
experiencia_titular = st.text_area('Experiencia Titular', value=st.session_state.experiencia_titular)
st.session_state.experiencia_titular = experiencia_titular
...
if "eje_proyecto" not in st.session_state:
    st.session_state.eje_proyecto = eje_proyecto_options[0]
eje_proyecto = st.selectbox('Eje del Proyecto', eje_proyecto_options, index=eje_proyecto_options.index(st.session_state.eje_proyecto))
st.session_state.eje_proyecto = eje_proyecto
...
if "beneficiarios_proyectos" not in st.session_state:
    st.session_state.beneficiarios_proyectos = []
beneficiarios_proyectos = st.multiselect('Beneficiarios del Proyecto', beneficiarios_proyectos_options, default=st.session_state.beneficiarios_proyectos)
st.session_state.beneficiarios_proyectos = beneficiarios_proyectos

# Step 4
if "numero_beneficiarios" not in st.session_state:
    st.session_state.numero_beneficiarios = 1
numero_beneficiarios = st.number_input('Número de Beneficiarios', min_value=1, format="%i", value=st.session_state.numero_beneficiarios)
st.session_state.numero_beneficiarios = numero_beneficiarios

# Step 5
if "inversion_proyecto" not in st.session_state:
    st.session_state.inversion_proyecto = 0.0
inversion_proyecto = st.number_input('Inversión del Proyecto', min_value=0.0, format="%f", value=st.session_state.inversion_proyecto)
...
def create_prompt_1(session_state):
    # Construct the prompt based on session state
    prompt_1 = f'''Role: You are an AI assistant trained in the formulation of creative and cultural economy projects using the logical framework methodology. Give your answers in Spanish.
                ...
                Task 1: Read the following documents {text_convocatoria} and {session_state.contexto_proyecto} to understand the context of the project. Do not write anything after performing this task. 
                
                Task 2: Analyze the following objectives {session_state.objetivo_proyecto} to understand the objectives of the project. Do not write anything after performing this task.
                
                Task 3: Analize the goals of the project that are {session_state.metricas_exito}. Do not write anything after performing this task.
                
                Task 4: Analyze the budget assign for the project {session_state.inversion_proyecto} in Colombian Pesos. Do not write anything after performing this task.
                
                Task 5: Take into account that {session_state.numero_beneficiarios} people will be positively impacted by the project. Do not write anything after performing this task.
                
                Task 6: The project is aligned with the following axis of the call {session_state.eje_proyecto}.Do not write anything after performing this task.
                
                Task 7: Write the detailed logical framework matrix as a plain text document for the project called {session_state.nombre_proyecto} that will benefit the population of {session_state.beneficiarios_proyectos}. The answer should have the following format: 
                        Task 7: 
...
def create_prompt_2(marco_logico, session_state):
    # Construct the prompt based on session state
...
def extract_tasks(response):
    tasks = {}
    for i in range(1, 9):
        match = re.search(f"Task {i}:(.*?)(Task {i + 1}:|$)", response["completion"], re.DOTALL)
        if match is not None:
            tasks[i] = match.group(1).strip()
        else:
            tasks[i] = f"Task {i} not found in text"
    return tasks
